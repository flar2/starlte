...
void auto_input_report(struct auto_input_data *data, unsigned int type, unsigned int code, int value)
{
	if (data->sync_end) {
		data->sync_end = false;
		if (code != ABS_MT_SLOT)
			input_report_abs(data->input_dev, ABS_MT_SLOT, data->prev_slot);
	}
...
static DEVICE_ATTR(enable, 0600, auto_input_enable_show, auto_input_enable_store);
static DEVICE_ATTR(tsp_event, 0200, NULL, auto_input_tsp_event_store);
